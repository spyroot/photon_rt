...
menuentry "Install" {
    linux /isolinux/vmlinuz root=/dev/ram0 ks=cdrom:/isolinux/ks.cfg loglevel=3 photon.media=UUID=\$photondisk photon.media.mount_retry=30
...
function main() {
  ...
  cp "$DEFAULT_RPM_DIR"/* "$dst_iso_dir"/"$DEFAULT_RPM_DST_DIR" >/dev/null 2>&1

  log "Copy git tar.gz from $DEFAULT_GIT_DIR to $dst_iso_dir / $DEFAULT_GIT_DST_DIR"
  cp "$DEFAULT_GIT_DIR"/* "$dst_iso_dir"/"$DEFAULT_GIT_DST_DIR" >/dev/null 2>&1

  log "Copy arcs from $DEFAULT_ARC_DIR to $dst_iso_dir / $DEFAULT_ARC_DST_DIR"
  cp "$DEFAULT_ARC_DIR"/* "$dst_iso_dir"/"$DEFAULT_ARC_DST_DIR" >/dev/null 2>&1
